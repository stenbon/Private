...
def _gemini_complete(system_prompt, user_content, max_tokens=8000):
    """Единая обёртка над Gemini 2.5 Flash + grounding (google_search) через
    Interactions API (client.interactions.create — актуальный, рекомендуемый
    Google способ на 08.2026, см. ai.google.dev/gemini-api/docs/google-search
    и .../interactions-overview). Модель сама решает, когда и что искать —
    результаты автоматически вплетены в текст ответа (interaction.output_text).

    12.08.2026: написана взамен _groq_complete из dzen_publisher.py — groq/compound
    на этом аккаунте стабильно падает с 413 request_too_large независимо от
    max_tokens (проверено на живых прогонах вплоть до max_tokens=2000), похоже
    дело в тарифе/квоте на встроенный поиск, не в размере запроса. У Gemini
    2.5 Flash бесплатный тариф отдельно считает grounding-квоту (5000
    запросов/месяц) — для 3 статей/день с запасом.

    Ретрай при ошибках квоты (429/RESOURCE_EXHAUSTED) — 2 повтора с паузой,
    т.к. свободный тариф Gemini считает лимиты по RPM (запросов в минуту),
    кратковременный отказ обычно проходит сам через 20-30 секунд."""
    last_err = None
...
<image_prompt>описание обложки на английском, фотореализм, без текста, без красных обводок/стрелок/восклицательных знаков, без гипертрофированной мимики лиц, 16:9</image_prompt>"""
        return _gemini_complete(system_prompt, user_content, max_tokens=8000)
