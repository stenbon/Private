...
Пайплайн: Google Sheets (тема) → groq/compound (текст + встроенный веб-поиск) →
fal.ai FLUX.1 schnell (обложка + 3 иллюстрации) → мультик из листа "Мультики" →
WordPress (REST API, Application Password) → RSS → Дзен

05.08.2026: генератор/фактчек переведены с Anthropic (claude-sonnet-5) на groq/compound —
счёт в 3$ за ночь на Anthropic API был признан неприемлемым.

12.08.2026: восстановлен из git-истории (был удалён 05.08.2026 при переходе на
Cowork-задачу 000l-dzen-daily-batch — решение оказалось ошибочным, Cowork-версия
зависит от включённого компьютера пользователя, из-за чего публикация вставала
на много часов каждый раз, когда компьютер выключен). При восстановлении:
- Ideogram (был нестабилен, 401-ошибки, привязан к отдельному аккаунту/квоте)
  заменён на fal.ai FLUX.1 schnell — тот же генератор картинок, что использует
  Cowork-пайплайн, дешёвый и надёжный на практике.
- Добавлены 3 иллюстрации в тело статьи (не только обложка) и вставка мультика
  в конце статьи по методологии, отработанной 11.08.2026 (лист "Мультики",
  обёртка <!-- wp:html --> вокруг всего текста + отдельный <!-- wp:video -->
  блок с /mp4/{id}.mp4-прокси — обходит баг ProZen, который иначе съедает
  текст статьи из RSS-ленты при смешении Gutenberg-блоков со свободным HTML).
- Обложка больше НЕ ставится как Featured Image — вставляется первой картинкой
  в тело статьи (как в Cowork-версии), чтобы не дублироваться визуально.

Требования:
    pip install groq requests python-dotenv gspread google-auth

Настройка GitHub Secrets:
    WP_URL, WP_USER, WP_APP_PASS, WP_CATEGORY, GROQ_API_KEY, GOOGLE_CREDENTIALS, FAL_API_KEY
"""
...
def generate_article(topic):
    print(f"[1/4] Генерирую текст с веб-поиском: «{topic}»...")

    system_prompt = ARTICLE_SYSTEM + """

КРИТИЧЕСКИ ВАЖНО — правила работы с фактами:
- Каждую конкретную цифру, процент, сумму или статистику ты ОБЯЗАН сначала найти через
  инструмент web_search в этом же запросе. Не пиши цифры по памяти или "на глаз".
- Если не нашёл подтверждение цифре через поиск — не используй эту цифру вообще.
  Пиши без неё, обычным текстом ("значительная часть", "заметно выросло" и т.п.).
- Никогда не пиши "по данным опросов", "по оценкам экспертов", "исследования показывают"
  без того, чтобы реально найти это исследование через web_search.
...
def publish_next():
    ...
    try:
        ...
        needs_review, problems = self_check_facts(article["html"])
        if needs_review and problems:
            article["html"] = fix_flagged_issues(article["html"], problems)
            needs_review, problems = self_check_facts(article["html"])
            if needs_review:
                print("    ⚠️ После попытки исправления фактчек всё ещё находит проблемы — уходит в черновик.")
        ...
        if multik_row and status == "publish":
