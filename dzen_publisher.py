...
def self_check_facts(html):
    ...
    try:
        response = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "Ты — строгий фактчекер. Проверяешь текст статьи на "
                    "выдуманные цифры, нереалистичные утверждения, "
                    "несуществующие сервисы/инструменты, противоречия. "
                    "Отвечай только по существу, кратко."
                )},
                {"role": "user", "content": f"""Проверь эту статью на фактические ошибки, выдуманные цифры и нереалистичные утверждения.

Если всё в порядке — ответь ровно: OK

Если есть проблемы — перечисли их списком, каждая проблема с новой строки, кратко (1 строка на проблему).
